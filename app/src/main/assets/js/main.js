...
var aa = define(['jquery'], function(jquery){
	function getGoodsInfo(){
		...
		mui.post(basePath + 'themeStoreGoodsInfo/queryGoodsByName',{
			token: token,
			keyWord: keyWord,
	...
	function init(){
		mui.post(basePath + 'themeStoreGoodsInfo/queryGoodsByThemeStoreId',{
			token:token,
			themeStoreId:themeStoreId
		},function(data){
			$(".indexlist").html("");
	...
	function selectGoods(obj){
		...
		mui.post(basePath + 'vendLayerTrackGoods/setTrackGoods', {
		    token: token,
		    layerIds: layerIds,
		    goodsId: id
		}, function(data){
		    if (data.success == true) {
		        window.location.href = 'goodsAtMachine.html?' + themeStoreId + ";" + layerIds +
		        ";" + index;
		    } else {
		        alert(data.message);
		    }
