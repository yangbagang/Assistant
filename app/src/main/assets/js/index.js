...
var aa = define(['jquery','swiper','mui','dialog'], function(jquery){
	function saveInfo(){//用来保存调换位置的信息
		...
		if(str != ""){
			mui.post(basePath + 'vendLayerTrackGoods/changePosition',{
				token:token,
				data:str,
	...
	function init(flag,isFirst){//初始化数据 加载主机数据
		...
		mui.post(basePath + 'vendLayerTrackGoods/getGoodsInfo',{
			token:token,
			themeStoreId:themeStoreId
		},function(data){
			if(data.success == true && data.status == "000"){
				...
				var layerNum = data.layerNum;//总共有几层
				var layerOrbitalArray = data.layerOrbitalNum;//各层轨道数
				var maxNum = layerOrbitalArray.max();
				$("#showData").append($("#zhujiHeader" + maxNum).html());
				var info = data.subList;//格子柜信息
				if(info != null && info !="" && !flag){
					...
					for(var i=0;i<info.length;i++){
						...
						if(gnum == 88){
							...
							$("#g05li").find(".mui-title").html(info[i].layer);
						...
						//	$("#g0804li").find(".chooseProduct").attr("onclick","gChooseProduct()");
							$("#g0804li").find(".mui-title").html(info[i].layer);
			...
			var qindex = index;
			if(qindex != null && flag == false){
				mySwiper.slideTo(qindex, 1000, true);
			}
		},'json'
		);
	};
	...
	function getSubData(sid,num,index,flag){//加载格子柜数据
		...
		mui.post(basePath + 'vendLayerTrackGoods/selectSubCabinetGoods',{
			token:token,
			sid:sid
		},function(data){
			if(data.success == true){
	...
	function openEmptyDoor(){//打开空柜门
		 ...
		 var sid = data.split("#")[0];
	 	 mui.post(basePath + 'vendLayerTrackGoods/openEmptyDoor',{
				token:token,
				themeStoreId:themeStoreId,
				sid:sid
		},function(data){
		...
		$(obj).find(".checked").each(function() {
		 ...
		 if(ids == ""){
			 ...
			 $("#sure1").on("tap",function(){
				 var index = mySwiper.activeIndex;
				 var obj = $("#slider").children().eq(index);
				 	 mui.post(basePath + 'vendLayerTrackGoods/openDoor',{
							token:token,
							themeStoreId:themeStoreId,
							trackNos:ids
					},function(data){
	...
	function setGnum(thisobj){//设置格子柜的数量
		...
		if(ids == ""){
			...
			mui.post(basePath + 'vendLayerTrackGoods/setGoodsNum',{
				token:token,
				layerIds:ids,
				num:num
			},function(data){
	...
	function chooseGZGoods(){//格子柜选择商品
		...
		if(ids == ""){
			...
			window.location.href = 'goodsList.html?' + themeStoreId+';' + ids + ";" + index;
		}
	};
